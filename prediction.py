...
def predict_player_from_csv(player_name, season, csv_filepath="nhl_historical_stats.csv", 
                            model_path="nhl_point_predictor.keras", model_path2="nhl_point_predictorv2.keras", model_path3="nhl_points_predictorv3.keras", model_path4="nhl_points_predictorv4.keras", scaler_path="scaler.pkl"):
    """
    ...
    alt_prediction3 = model4.predict(scaled_stats, verbose=0)

    
    # Return predicted value (clamped to 0 minimum)
    return (max(0.0, prediction[0][0]), alt_prediction[0][0], alt_prediction2[0][0], alt_prediction3[0][0], stats_array)
...
if __name__ == "__main__":
    ...
    result4 = prediction[3]
    
    if isinstance(result1, str):
        print(f"Stats for {RED}{target_player}{RESET} for the {YELLOW}{base_season-1}-{base_season}{RESET} season were not able to be found.")
        print("Either the player does not exist or has already retired by this point")
    else:
        print("\n" + "="*50)
        ...
        print(f"Projected Points for {YELLOW}{base_season}-{base_season+1}{RESET} Season (500 Epochs | Reduced +/-): {result4:.1f}")
        print("="*50)